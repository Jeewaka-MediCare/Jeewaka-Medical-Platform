...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(fileExt)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only PDF, JPG, PNG, DOC, DOCX files are allowed."
        )
      );
    }
  },
  ...
  try {
    ...
    if (existingRequest) {
      return res
        .status(400)
        .json({
          message: "Verification request already exists for this doctor.",
        });
    }
...
export const getAllVerifications = async (req, res) => {
  try {
  const verificationsRaw = await adminVerificationSchema
  .find()
...
export const getVerificationByDoctorId = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get verification", error: error.message });
  }
};
...
export const deleteDoctorDocument = async (req, res) => {
  try {
    ...
    if (!doctorId || !filename) {
      return res
        .status(400)
        .json({ message: "Doctor ID and filename are required" });
    }

    const result = await storageService.deleteDoctorDocument(
      doctorId,
      filename
    );

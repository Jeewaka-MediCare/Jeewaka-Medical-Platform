  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      try {
        ...
        console.error(
          "⚠️ Doctor created but failed to send email:",
          emailError.message
        );
      }
    ...
    return res
      .status(500)
      .json({ error: "Failed to insert doctor(s)", details: error.message });
  }
};
...
export const searchDoctors = async (req, res) => {
  try {
    ...
    if (minExperience || maxExperience) {
      ...
      if (minExperience)
        searchQuery.yearsOfExperience.$gte = parseInt(minExperience);
      if (maxExperience)
        searchQuery.yearsOfExperience.$lte = parseInt(maxExperience);
    }
...
export const getFilterOptions = async (req, res) => {
  try {
    const [specializations, subSpecializations, languages] = await Promise.all([
      Doctor.distinct("specialization").then((results) =>
        results.filter(Boolean)
      ),
      Doctor.distinct("subSpecializations").then((results) =>
        results.filter(Boolean)
      ),
      Doctor.distinct("languagesSpoken").then((results) =>
        results.filter(Boolean)
      ),
    ]);
    ...
    res.json({
      ...
      data: {
        ...
        experienceRange: experienceStats[0] || {
          minExperience: 0,
          maxExperience: 0,
        },
...
function basicKeywordFallback(query) {
  ...
  if (
    ...
    numbers &&
    (lowerQuery.includes("above") || lowerQuery.includes("over"))
  ) {
...
function getFallbackSuggestions(partialQuery) {
  ...
  const filtered = commonSuggestions.filter(
    (suggestion) =>
